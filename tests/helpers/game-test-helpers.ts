...
import type { GameResult, TimeControl } from '../../src/types/game.types.js';
import {
  TEST_PLAYERS,
  GAME_RESULTS,
  CHESS_POSITIONS,
...
export function createTestGame(
  whitePlayerId: string = TEST_PLAYERS.WHITE_PLAYER,
  blackPlayerId: string = TEST_PLAYERS.BLACK_PLAYER,
 ...
 */
export function expectBasicGameProperties(
  game: Game,
 ...
 */
export function expectInitialGameState(game: Game): void {
 ...
 */
export function expectTimeControlInitialization(
  game: Game,
  timeControl?: TimeControl
): void {
  if (timeControl) {
 ...
 */
export function expectUniqueGameIds(games: Game[]): void {
 ...
 */
export function expectValidTimestamps(game: Game, beforeCreation?: Date): void {
  expect(game.createdAt).toBeInstanceOf(Date);
  expect(game.updatedAt).toBeInstanceOf(Date);
  ...
  if (beforeCreation) {
    ...
    expect(game.createdAt.getTime()).toBeLessThanOrEqual(
 ...
 */
export function expectGameCompletion(
  game: Game,
 ...
 */
export function expectActiveGames(
  gameManager: GameManager,
 ...
 */
export function expectCompletedGames(
  gameManager: GameManager,
  expectedCount: number,
  expectedGames?: Game[]
): void {
 ...
 */
export function expectAllGames(
  gameManager: GameManager,
  expectedCount: number,
  expectedGames?: Game[]
): void {
 ...
 */
export async function expectConcurrentGameLimit(
  gameManager: GameManager,
  maxGames: number
): Promise<void> {
  ...
  await expect(async () => {
    ...
    `Maximum number of concurrent games (${maxGames}) reached`
  );
}
 ...
 */
export async function createMixedGameScenario(
  gameManager: GameManager
): Promise<{
  activeGames: Game[];
  completedGames: Game[];
  ...
  const game1 = await gameManager.createGame(
    TEST_PLAYERS.WHITE_PLAYER,
    TEST_PLAYERS.BLACK_PLAYER
  );
  const game2 = await gameManager.createGame(
    TEST_PLAYERS.PLAYER_3,
    TEST_PLAYERS.PLAYER_4
  );
  const game3 = await gameManager.createGame(
    TEST_PLAYERS.PLAYER_5,
    TEST_PLAYERS.PLAYER_6
  );
